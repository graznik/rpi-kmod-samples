...
static irqreturn_t button_isr(int irq, void *data)
{
	...
	if (irq == button_irqs[0]) {
		...
		/* wait for BUTTON release, but no more then 5 x mdelay(5) */
		while (gpio_get_value(buttons[0].gpio) && max_delay--)
			mdelay(5);
		...
		while(gpio_get_value(buttons[1].gpio) && max_delay--)
			mdelay(5);
	}
